...
class Stats {
  ...
  drawOverallStats() {
    ...
    this.ctx.beginPath();
    this.ctx.roundRect(this.padding, cardY, cardW, cardH, 12);
    this.ctx.fill();
  ...
  drawGameStats() {
    ...
    this.games.forEach((gameName, i) => {
      ...
      this.ctx.beginPath();
      this.ctx.roundRect(this.padding, itemY, cardW, itemH - 6, 10);
      this.ctx.fill();
      ...
      this.ctx.fillStyle = winRateColor;
      this.ctx.beginPath();
      this.ctx.roundRect(barX, barY, barW * (winRate / 100), barH, 3);
      this.ctx.fill();
