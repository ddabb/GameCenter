/**
 * achievement-manager.js — 成就系统 v2.1 (修复版)
 * 
 * 修复内容：
 * 1. _enrichProgress() - 修复通用成就target计算，修复全局成就进度计算
 * 2. getByGame() - 重写分组逻辑，确保成就正确归到对应游戏
 * 3. 保留原有接口，向后兼容
 */
...
class AchievementManager {
  ...
  check(gameName, winCount) {
    ...
    // "多面手"：每个游戏至少1个独有成就
    const hasOneEach = GAMES.every(g => {
      const uniqueList = UNIQUE_ACHIEVEMENTS[g] || [];
      return uniqueList.some(a => this.data.unlocked[a.id]);
    });
  ...
  _enrichProgress(a) {
    // 通用成就（按游戏）
    const genMatch = a.id.match(/^(first_win|win_10|win_50|win_100)_(.+)$/);
    if (genMatch) {
      const suffix = genMatch[1];
      const game = genMatch[2];
      const targetMap = { 'first_win': 1, 'win_10': 10, 'win_50': 50, 'win_100': 100 };
      const target = targetMap[suffix] || 1;
    ...
    if (globalTargets[a.id] !== undefined) {
      let progress = 0;
      if (a.id.startsWith('total_')) {
        // 计算总通关数
        GAMES.forEach(g => { progress += this._getWinCount(g); });
      } else if (a.id.startsWith('checkin_')) {
        // 从storage读取连续签到天数
        try {
          const d = JSON.parse(wx.getStorageSync('checkin') || '{}');
          progress = d.streak || 0;
        } catch (e) {}
      }
      return { ...a, progress, target: globalTargets[a.id] };
    }
    // 独有成就：无进度条（解锁即完成）
    return { ...a, progress: a.unlocked ? 1 : 0, target: 1 };
  }
  ...
  getByGame() {
    ...
    this.getAll().forEach(a => {
      // 1. 全局成就
      if (GLOBAL_ACHIEVEMENTS.find(g => g.id === a.id)) {
        result._global.push(a);
        return;
      }
      
      // 2. 通用成就（first_win_xxx, win_10_xxx 等）
      const genMatch = a.id.match(/^(first_win|win_10|win_50|win_100)_(.+)$/);
      if (genMatch) {
        const game = genMatch[2];
        if (!result._unique[game]) result._unique[game] = [];
        result._unique[game].push(a);
        return;
      }
      
      // 3. 独有成就（xxx_yyy）
      const uniqueMatch = a.id.match(/^([^_]+)_(.+)$/);
      if (uniqueMatch) {
        const game = uniqueMatch[1];
        if (UNIQUE_ACHIEVEMENTS[game]) {
          if (!result._unique[game]) result._unique[game] = [];
          result._unique[game].push(a);
          return;
        }
