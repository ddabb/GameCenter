...
function runPreview() {
  ...
  const compileType = fs.existsSync(gameJsonPath) && !fs.existsSync(appJsonPath) ? 'game' : 'miniprogram'
...
function main() {
  ...
  // 2. 检查项目路径（兼容小程序 app.json 和小游戏 game.json）
  const appJsonPath = path.join(PROJECT_PATH, 'app.json')
  const gameJsonPath = path.join(PROJECT_PATH, 'game.json')
  if (!fs.existsSync(appJsonPath) && !fs.existsSync(gameJsonPath)) {
    console.error('app.json 和 game.json 均不存在: ' + PROJECT_PATH)
